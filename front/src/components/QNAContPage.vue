  ...
  <v-main>
    <v-content class="mx-15">
      ...
      >
        ...
        <v-col>
          <span class="mytitle mt-3 ml-2">답변</span>
        </v-col>
        <v-divider class="mx-4"></v-divider>

        <v-col>
          <span class="ml-2">{{result[0].answer}}</span>
        </v-col>
        <v-divider class="mx-4 mb-4"></v-divider>
        
        <v-col class="mb-3 text-right">
          <v-btn v-if="this.$store.state.user >= 1" large color="error" @click='sakjei'>
            게시글 삭제
          </v-btn>
          <v-btn v-if="this.$store.state.user >= 1" large color="success" @click='soojung(result[0])'>
            게시글 수정
          </v-btn>
          <v-btn large color="info" @click='toQNA'>
            목록
          </v-btn>
        </v-col>
      </v-card>
